...
public class born_reg extends AppCompatActivity {
        ...
        b.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(connected){
                    ...
                    if(clgn.isEmpty()==true||mob.isEmpty()==true){
                        ...
                        obs.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                dataSnapshot = dataSnapshot.child("borntowander");
                                boolean flag = true;
                                if(dataSnapshot.hasChildren()){
                                    for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                                        ...
                                        if(p.equals(name)){
                                            ...
                                            Toast.makeText(born_reg.this, "Registration not done",
                                ...
                                if(flag){
                                    ...
                                    obs.child("borntowander").child(id).child("teamname").setValue(name);
                                    obs.child("borntowander").child(id).child("member1").setValue(mem1);
                                    obs.child("borntowander").child(id).child("member2").setValue(mem2);
                                    obs.child("borntowander").child(id).child("member3").setValue(mem3);

                                    obs.child("borntowander").child(id).child("email").setValue(email);
                                    obs.child("borntowander").child(id).child("mobile").setValue(mob);
                    ...
                    Toast.makeText(born_reg.this,"INTERNET CONNECTION IS NOT AVAILABLE",
                            Toast.LENGTH_LONG).show();
