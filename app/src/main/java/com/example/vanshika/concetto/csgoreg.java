...
public class csgoreg extends AppCompatActivity {
        ...
        b.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(connected){
                    ...
                    if(name.isEmpty()==true||mem1.isEmpty()==true||mob.isEmpty()==true||email.isEmpty()==true||clgn.isEmpty()==true){
                        ...
                        obs.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                dataSnapshot = dataSnapshot.child("cs-go");
                                boolean flag = true;
                                if(dataSnapshot.hasChildren()){
                                    for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                                        ...
                                        if(p.equals(name)){
                                            ...
                                            Toast.makeText(csgoreg.this, "Registration not done",
                                ...
                                if(flag){
                                    ...
                                    obs.child("cs-go").child(id).child("teamname").setValue(name);
                                    obs.child("cs-go").child(id).child("member1").setValue(mem1);
                                    obs.child("cs-go").child(id).child("member2").setValue(mem2);
                                    obs.child("cs-go").child(id).child("member3").setValue(mem3);
                                    obs.child("cs-go").child(id).child("member4").setValue(mem4);
                                    obs.child("cs-go").child(id).child("email").setValue(email);
                                    obs.child("cs-go").child(id).child("mobile").setValue(mob);
                    ...
                    Toast.makeText(csgoreg.this,"INTERNET CONNECTION IS NOT AVAILABLE",
                            Toast.LENGTH_LONG).show();
