...
public class spardha_reg extends AppCompatActivity {
        ...
        b.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(connected){
                    ...
                    if(clgn.isEmpty()==true||mob.isEmpty()==true||mem1.isEmpty()==true||mem2.isEmpty()==true){

                    }else {
                        obs.addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                dataSnapshot = dataSnapshot.child("spardha");
                                boolean flag = true;
                                if(dataSnapshot.hasChildren()){
                                    for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                                        ...
                                        if(p.equals(name)){
                                            ...
                                            Toast.makeText(spardha_reg.this, "Registration not done",
                                ...
                                if(flag){
                                    ...
                                    obs.child("spardha").child(id).child("teamname").setValue(name);
                                    obs.child("spardha").child(id).child("member1").setValue(mem1);
                                    obs.child("spardha").child(id).child("member2").setValue(mem2);
                                    obs.child("spardha").child(id).child("member3").setValue(mem3);

                                    obs.child("spardha").child(id).child("email").setValue(email);
                                    obs.child("spardha").child(id).child("mobile").setValue(mob);
                    ...
                    Toast.makeText(spardha_reg.this,"INTERNET CONNECTION IS NOT AVAILABLE",
                            Toast.LENGTH_LONG).show();
