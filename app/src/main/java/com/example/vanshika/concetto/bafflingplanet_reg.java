...
public class bafflingplanet_reg extends AppCompatActivity {
        ...
        b.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(connected){
                    ...
                    if(name.isEmpty()==true||mem1.isEmpty()==true||mem2.isEmpty()==true||mem3.isEmpty()==true||email.isEmpty()==true||clgn.isEmpty()==true){
                        ...
                        obs.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                dataSnapshot = dataSnapshot.child("baffling");
                                boolean flag = true;
                                if(dataSnapshot.hasChildren()){
                                    for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                                        ...
                                        if(p.equals(name)){
                                            ...
                                            Toast.makeText(bafflingplanet_reg.this, "Registration not done",
                                ...
                                if(flag){
                                    ...
                                    obs.child("baffling").child(id).child("teamname").setValue(name);
                                    obs.child("baffling").child(id).child("member1").setValue(mem1);
                                    obs.child("baffling").child(id).child("member2").setValue(mem2);
                                    obs.child("baffling").child(id).child("member3").setValue(mem3);
                                    obs.child("baffling").child(id).child("member4").setValue(mem4);
                                    obs.child("baffling").child(id).child("email").setValue(email);
                                    obs.child("baffling").child(id).child("mobile").setValue(mob);
                    ...
                    Toast.makeText(bafflingplanet_reg.this,"INTERNET CONNECTION IS NOT AVAILABLE",
                            Toast.LENGTH_LONG).show();
