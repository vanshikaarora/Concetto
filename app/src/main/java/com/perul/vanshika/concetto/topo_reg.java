...
public class topo_reg extends AppCompatActivity {
        ...
        b.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(connected){
                    ...
                    if(name.isEmpty()==true||mem1.isEmpty()==true||email.isEmpty()==true||clgn.isEmpty()==true){
                        ...
                        obs.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                dataSnapshot = dataSnapshot.child("topographia");
                                boolean flag = true;
                                if(dataSnapshot.hasChildren()){
                                    for(DataSnapshot snapshot : dataSnapshot.getChildren()){
                                        ...
                                        if(p.equals(name)){
                                            ...
                                            Toast.makeText(topo_reg.this, "Registration not done",
                                ...
                                if(flag){
                                    ...
                                    obs.child("topographia").child(id).child("teamname").setValue(name);
                                    obs.child("topographia").child(id).child("member1").setValue(mem1);
                                    obs.child("topographia").child(id).child("member2").setValue(mem2);
                                    obs.child("topographia").child(id).child("member3").setValue(mem3);
                                    obs.child("topographia").child(id).child("email").setValue(email);
                                    obs.child("topographia").child(id).child("mobile").setValue(mob);
                                    obs.child("topographia").child(id).child("college").setValue(clgn);

                                    FirebaseMessaging.getInstance().subscribeToTopic("surveytopographia");
                                    Toast.makeText(topo_reg.this, "Registration done FOR SURVEY TOPOGRAPHIA  and you will receive notifications regarding it. ",
                                            Toast.LENGTH_LONG).show();
                                    finish();
                                    startActivity(new Intent(topo_reg.this, MainActivity.class));
                    ...
                    Toast.makeText(topo_reg.this,"INTERNET CONNECTION IS NOT AVAILABLE",
                            Toast.LENGTH_LONG).show();
